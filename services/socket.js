...
function initSocket(httpServer, corsOrigins = ["*"]) {
  ...
  ioInstance.on("connection", (socket) => {
    ...
    socket.on("join_conversation", ({ id_conversacion }) => {
      ...
      console.log(
        `[socket] Cliente ${socket.id} unido a conv:${id_conversacion}`
      );
    });
    ...
    socket.on("typing_start", ({ id_conversacion, user_id }) => {
      ...
      socket
        .to(`conv:${id_conversacion}`)
        .emit("typing", { id_conversacion, user_id, typing: true });
    });
    socket.on("typing_stop", ({ id_conversacion, user_id }) => {
      if (!id_conversacion || !user_id) return;
      socket
        .to(`conv:${id_conversacion}`)
        .emit("typing", { id_conversacion, user_id, typing: false });
    });

    // Send message via WS -> persist -> broadcast
    socket.on("message_send", async (payload, ack) => {
      try {
        const { id_conversacion, id_usuario_sender, mensaje, media_url } =
          payload || {};
        if (
          !id_conversacion ||
          !id_usuario_sender ||
          ...
          if (ack)
            ack({
              success: false,
              message:
                "Campos requeridos: id_conversacion, id_usuario_sender y mensaje o media_url",
            });
          return;
        }
        ...
        ioInstance
          .to(`conv:${id_conversacion}`)
          .emit("message_new", { mensaje: saved });
